...
function addPortico(x, y, z, rotY, lintel) {
  const g = new THREE.Group();
  const dark = STUDIO_DARK;
  const goldM = STUDIO_GOLD;
  const tealM = STUDIO_TEAL;
  const left = musBox(2.2, 16.4, 2.4, dark);
  left.position.set(0, 8.2, -13.6);
  const right = musBox(2.2, 16.4, 2.4, dark);
  right.position.set(0, 8.2, 13.6);
  const wingL = musBox(1.6, 14.2, 10.5, dark);
  wingL.position.set(-0.2, 7.1, -20.4);
  const wingR = musBox(1.6, 14.2, 10.5, dark);
  wingR.position.set(-0.2, 7.1, 20.4);
  const beam = musBox(2.6, 1.5, 30.4, goldM);
  beam.position.set(0.15, 16.4, 0);
  const cornice = musBox(2.9, 0.38, 32.2, goldM);
  cornice.position.set(0.25, 17.25, 0);
  const baseL = musBox(2.8, 0.55, 12.2, tealM);
  baseL.position.set(0, 0.28, -20);
  const baseR = musBox(2.8, 0.55, 12.2, tealM);
  baseR.position.set(0, 0.28, 20);
  const capL = musBox(2.5, 0.45, 2.7, goldM);
  capL.position.set(0, 16.55, -13.6);
  const capR = musBox(2.5, 0.45, 2.7, goldM);
  capR.position.set(0, 16.55, 13.6);
  g.add(left, right, wingL, wingR, beam, cornice, baseL, baseR, capL, capR);
...
function addRackWall(x, y, z, rotY, lintel, cols, rows) {
  const g = new THREE.Group();
  ...
  const base = musBox(1.8, 0.4, 23.2, STUDIO_TEAL);
  base.position.set(0, 0.2, 0);
  g.add(wall, beam, base);
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const frame = musBox(0.12, 3.4, 4.4, STUDIO_GOLD);
      frame.position.set(0.62, 12.2 - r * 4.15, -7.2 + c * 7.2);
      const pane = musBox(0.06, 3.05, 4.0, STUDIO_TEAL);
      pane.position.set(0.7, 12.2 - r * 4.15, -7.2 + c * 7.2);
      g.add(frame, pane);
    }
  }
  const lab = makeLabel(lintel, {
...
function addArrivalAnchors() {
  // West: mid-distance studio portico on the path to the real facade.
  addPortico(-42, 0, -6, 0, "STUDIO");
  // East: research rack, offset north of the +X lane.
  addRackWall(46, 0, 18, Math.PI, "RESEARCH", 3, 3);
  // North: writing portico on the +Z lane.
  addPortico(0, 0, 54, Math.PI / 2, "WRITING");
}

async function populateNetarts() {
  ...
  NETARTS_CARDS.forEach((card, i) => {
    const m = makeCard(card[0], card[1]);
    const row = i;
    m.position.set(wx - 14.6, wy + 9.2 - row * 3.55, wz + 0.12);
...
async function populate() {
  ...
  addArrivalAnchors();
  await populateNetarts();

  await populateStudio();
...
async function main() {
  ...
  scene.add(far);
  const westGate = new THREE.PointLight(0xd4b05a, 20, 90);
  westGate.position.set(-40, 16, 0);
  scene.add(westGate);
  const eastRack = new THREE.PointLight(0x2aa8a0, 16, 80);
  eastRack.position.set(46, 14, 18);
  scene.add(eastRack);
  const northHall = new THREE.PointLight(0xd4b05a, 14, 80);
  northHall.position.set(0, 14, 52);
  scene.add(northHall);
