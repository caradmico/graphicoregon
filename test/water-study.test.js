...
assert.ok(w.headR > 0.12 && w.headR < 0.20, "head has oval mass, not a pin or a snowman orb");
...
assert.ok(mass.torsoMaxR > 0.12, "torso lathe has chest mass");
assert.ok(mass.hipMaxR > 0.15, "hips have seated mass");
assert.ok(mass.headMaxR > 0.08, "head lathe is an oval, not a stick tip");
assert.ok(mass.thighR > 0.05, "thighs have thickness");
assert.ok(mass.upperArmR > 0.04, "arms have thickness");
assert.ok(mass.depth > 0.7 && mass.depth < 0.95, "body is volumetric, not a paper-thin card");
