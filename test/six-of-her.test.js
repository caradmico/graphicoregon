...
assert.ok(/canvas#stage[\s\S]*visibility:\s*visible/.test(css), "the canvas is first paint, not a hidden wait");
assert.ok(/function showStage/.test(app) && /showStage\(\)/.test(app), "JS shows the stage on first paint");
